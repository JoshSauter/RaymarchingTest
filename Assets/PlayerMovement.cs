...
#region IsGrounded characteristics
	...
	void FixedUpdate() {
		...
		if (grounded) {
			...
			// Handle jumping
			if (Input.GetKey(KeyCode.Space) && !jumpIsOnCooldown) {
				StartCoroutine(Jump());
			}
		}
		else {
	...
	Vector3 CalculateGroundMovement(RaycastHit ground) {
        ...
        Vector2 leftStick = (Input.GetKey(KeyCode.W) ? 1 : 0) * Vector2.up +
	...
	Vector3 CalculateAirMovement() {
        ...
        Vector3 moveDirection = leftStick.y * transform.forward + leftStick.x * transform.right;
	...
	public bool IsGrounded(out RaycastHit hitInfo) {
		...
		foreach (RaycastHit curHitInfo in allHit) {
            ...
            if (curHitInfo.collider.CompareTag("Player") || Physics.GetIgnoreLayerCollision(gameObject.layer, curHitInfo.collider.gameObject.layer)) continue;
